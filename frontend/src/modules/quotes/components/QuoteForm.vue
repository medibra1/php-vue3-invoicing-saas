...
const { save, isLoading, errorMessage } = useQuotes()
...
const items = reactive<QuoteItemPayload[]>([{ description: '', quantity: 1, unit_price: 0 }])
...
onMounted(async () => {
  ...
  try {
    const quote = await quotesStore.fetchOne(id)
    clientId.value = quote.client_id
    notes.value = quote.notes ?? ''
    expiryDate.value = quote.expiry_date ?? ''
    items.splice(
      0,
      items.length,
...
<template>
  <div class="px-6 py-8">
    <div class="mx-auto max-w-2xl">
      <Card>
        <h1 class="mb-6 text-xl font-medium text-gray-900">{{ isEdit ? 'Edit quote' : 'New quote' }}</h1>
        ...
        <form v-else class="space-y-6" @submit.prevent="onSubmit">
          ...
          <Input v-model="expiryDate" label="Expiry date" type="date" />
